...
class Gripper(object):
    ...
    def move(self, steps, wait=True, timeout=2):
        ...
        for servo_id, servo in self.servos.items():
            ...
            dxl_result = self.group_bulk_write.addParam(servo_id, servo.addresses["goal_position"],
                                                        servo.addresses["goal_position_length"], param_goal_position)
    ...
    @exception_handler("Failed while trying to move object by steps")
    def move_object(self, steps, wait=True, timeout=2):
        if not self.verify_steps(steps):
            error_message = f"Gripper#{self.gripper_id}: The move command provided is out of bounds: Step {steps}"
            logging.error(error_message)
            raise ValueError(error_message)
    ...
    def set_velocity(self, velocities):
        ...
        for servo_id, servo in self.servos.items():
            ...
            param_goal_velocity = servo.data_to_bytes(target_velocity_b,
                                                      servo.addresses[self.velocity_in_pos_control + "_length"])
            dxl_result = self.group_bulk_write.addParam(servo_id, servo.addresses[self.velocity_in_pos_control],
                                                        servo.addresses[self.velocity_in_pos_control + "_length"],
                                                        param_goal_velocity)
    ...
    def move_velocity_wheel(self, velocities):
        ...
        for servo_id, servo in self.servos.items():
            ...
            param_goal_velocity = servo.data_to_bytes(target_velocity,
                                                      servo.addresses[self.velocity_in_pos_control + "_length"])
            dxl_result = self.group_bulk_write.addParam(servo_id, servo.addresses[self.velocity_in_pos_control],
                                                        servo.addresses[self.velocity_in_pos_control + "_length"],
                                                        param_goal_velocity)
    ...
    def set_operating_mode(self, new_mode):
        if (new_mode != self.current_operating_mode()).all():
            ...
            for servo_id, servo in self.servos.items():
                ...
                dxl_result = self.group_bulk_write.addParam(servo_id, servo.addresses["operating_mode"],
                                                            servo.addresses["operating_mode_length"],
                                                            [new_mode[servo_id - 1]])
    ...
    def enable_torque(self):
        for servo_id, servo in self.servos.items():
            dxl_result = self.group_bulk_write.addParam(servo_id, servo.addresses["torque_enable"],
                                                        servo.addresses["torque_enable_length"], [1])
    ...
    def disable_torque(self):
        for servo_id, servo in self.servos.items():
            dxl_result = self.group_bulk_write.addParam(servo_id, servo.addresses["torque_enable"],
                                                        servo.addresses["torque_enable_length"], [0])
    ...
    def change_LED_colours(self, color):
        for servo_id, servo in self.servos.items():
            servo.LED_colour = color
            servo.turn_on_LED()
    ...
    def check_bulk_read_avaliability(self, servo_id, address, length):
        dxl_getdata_result = self.group_bulk_read.isAvailable(servo_id, self.servos[servo_id].addresses[address],
                                                              length)
