    ...
    def decorator(function):
        ...
        def wrapper(self, *args, **kwargs):
            try:
                ...
                raise DynamixelServoError(error.servo,
                                          message=f"Dynamixel#{error.servo.motor_id}: {error_message}") from error
...
class Servo(object):
    def __init__(self, port_handler, packet_handler, protocol, motor_id, LED_colour, torque_limit, max_velocity, max,
                 min, model="XL-320"):
    ...
    def operating_mode(self):
        operating_mode, dxl_comm_result, dxl_error = self.packet_handler.read1ByteTxRx(self.port_handler, self.motor_id,
                                                                                       self.addresses["operating_mode"])
        self.process_result(dxl_comm_result, dxl_error,
                            message=f"Dynamixel#{self.motor_id}: successfully read operating mode as {operating_mode}")
    ...
    def set_operating_mode(self, new_mode):
        ...
        dxl_comm_result, dxl_error = self.packet_handler.write1ByteTxRx(self.port_handler, self.motor_id,
                                                                        self.addresses["operating_mode"], new_mode)
    ...
    def move(self, target_position, wait=True, timeout=5):
        ...
        if self.addresses["goal_position_length"] == 2:
            ...
            dxl_comm_result, dxl_error = self.packet_handler.write2ByteTxRx(self.port_handler, self.motor_id,
                                                                            self.addresses["goal_position"],
                                                                            target_position)
        elif self.addresses["goal_position_length"] == 4:
            self.set_operating_mode(OperatingMode.POSITION.value)
            dxl_comm_result, dxl_error = self.packet_handler.write4ByteTxRx(self.port_handler, self.motor_id,
                                                                            self.addresses["goal_position"],
                                                                            target_position)
    ...
    @exception_handler("Failed while moving")
    def move_extended(self, target_position, wait=True, timeout=5):
        if not self.verify_step(target_position):
            error_message = f"Dynamixel#{self.motor_id}: Target position {target_position} is out of bounds of min {self.min} max {self.max}"
            logging.error(error_message)
            raise DynamixelServoError(self, error_message)

        self.set_operating_mode(OperatingMode.EXTENDED_POSITION.value)
        dxl_comm_result, dxl_error = self.packet_handler.write4ByteTxRx(self.port_handler, self.motor_id,
                                                                        self.addresses["goal_position"],
                                                                        target_position)
    ...
    def current_position(self):
        if self.addresses["current_position_length"] == 2:
            ...
            data_read, dxl_comm_result, dxl_error = self.packet_handler.read4ByteTxRx(self.port_handler, self.motor_id,
                                                                                      self.addresses[
                                                                                          "current_position"])

        self.process_result(dxl_comm_result, dxl_error,
                            message=f"Dynamixel#{self.motor_id}: measured position {data_read}")
        return data_read
    ...
    def current_goal_position(self):
        ...
        self.process_result(dxl_comm_result, dxl_error,
                            message=f"Dynamixel#{self.motor_id}: current goal position {data_read}")
        return data_read
        ...
        if self.addresses["current_velocity_length"] == 2:
            ...
            data_read, dxl_comm_result, dxl_error = self.packet_handler.read4ByteTxRx(self.port_handler, self.motor_id,
                                                                                      self.addresses[
                                                                                          self.velocity_in_pos_control])
        self.process_result(dxl_comm_result, dxl_error,
                            message=f"Dynamixel#{self.motor_id}: measured velocity {data_read}")
        return data_read

    @exception_handler("Failed to read current load")
    def current_load(self):
        data_read, dxl_comm_result, dxl_error = self.packet_handler.read2ByteTxRx(self.port_handler, self.motor_id,
                                                                                  self.addresses["current_load"])
        self.process_result(dxl_comm_result, dxl_error,
                            message=f"Dynamixel#{self.motor_id}: current load is {data_read}")
    ...
    def read_shutdown(self):
        data_read, dxl_comm_result, dxl_error = self.packet_handler.read1ByteTxRx(self.port_handler, self.motor_id,
                                                                                  self.addresses["shutdown"])
        self.process_result(dxl_comm_result, dxl_error,
                            message=f"Dynamixel#{self.motor_id}: measured shutdown status {data_read}")
    ...
    def read_hardware_errors(self):
        ...
        self.process_result(dxl_comm_result, dxl_error,
                            message=f"Dynamixel#{self.motor_id}: measured hardware error status {data_read}")
    ...
    def turn_on_LED(self):
        dxl_comm_result, dxl_error = self.packet_handler.write1ByteTxRx(self.port_handler, self.motor_id,
                                                                        self.addresses["led"], self.LED_colour)
        self.process_result(dxl_comm_result, dxl_error,
    ...
    def enable_torque(self):
        dxl_comm_result, dxl_error = self.packet_handler.write2ByteTxRx(self.port_handler, self.motor_id,
                                                                        self.addresses["torque_enable"], 1)
        self.process_result(dxl_comm_result, dxl_error,
    ...
    def disable_torque(self):
        dxl_comm_result, dxl_error = self.packet_handler.write1ByteTxRx(self.port_handler, self.motor_id,
                                                                        self.addresses["torque_enable"], 0)
        self.process_result(dxl_comm_result, dxl_error,
    ...
    def reboot(self):
        ...
        self.process_result(dxl_comm_result, dxl_error,
                            message=f"Dynamixel#{self.motor_id}: has successfully rebooted servo")
    ...
    def data_to_bytes(self, data, length):
        if length == 2:
            ...
            return [dxl.DXL_LOBYTE(dxl.DXL_LOWORD(data)), dxl.DXL_HIBYTE(dxl.DXL_LOWORD(data)),
